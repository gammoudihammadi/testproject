...
{
    public class InventoryAccountingPage : PageBase
    {

        public InventoryAccountingPage(IWebDriver webDriver, TestContext testContext) : base(webDriver, testContext)
        {
        }
        ...
        public double GetInventoryGrossAmount(string writingType, string decimalSeparator)
        {
        ...
        public double GetInventoryDetailAmount(string writingType, string decimalSeparator)
        {
            // Récupération du type de séparateur (, ou . selon les pays)
            CultureInfo ci = decimalSeparator.Equals(",") ? new CultureInfo("fr-FR") : new CultureInfo("en-US");
